...
public class AstralBlackTowerLot extends AstralStructureLot {
	...
	private final static DyeColor windowPrimaryColor = DyeColor.BLACK;
	private final static DyeColor windowSecondaryColor = DyeColor.PURPLE;
	private final static int mazeFloorHeight = 4;
	private final static int mazeWidth = 9; 
	private final static int mazeBottom = mazeFloorHeight;
	...
	protected void generateActualBlocks(WorldGenerator generator,
		...
		int y1 = mazeBottom;
		int y2 = generator.seaLevel + mazeFloorHeight * 10;
		y2 = y2 / mazeFloorHeight * mazeFloorHeight + mazeBottom + 1;
				...
				for (int n = 1; n < mazeWidth; n++)
					if (floor.getBit(m, n) == MazeBit.HALL) {
						...
						if (chunk.isType(x1, y1 + 1, z1, wallMaterial)) {
							...
							chunk.setBlocks(x1, x1 + 2, y1 + 1, y1 + mazeFloorHeight, z1, z1 + 2, hallMaterial);
							
							if (hallMaterial == emptyHallMaterial && !chunk.isEmpty(x1, y1, z1)) {
								int x = x1 + chunkOdds.getRandomInt(2);
								int z = z1 + chunkOdds.getRandomInt(2);
								chunk.setSlabs(x, x + 1, y1 + 1, y1 + 2, z, z + 1, Material.COBBLESTONE, false);
								
							} else if (hallMaterial == specialHallMaterial) {
								for (int y = y1 + 1; y < y1 + mazeFloorHeight; y++) {
									int x = x1 + chunkOdds.getRandomInt(2);
									int z = z1 + chunkOdds.getRandomInt(2);
		...
		// notch the corners
		int y3 = generator.seaLevel + mazeFloorHeight * 2;
		chunk.setBlocks(0, y3, y1, 0, Material.AIR);
		chunk.setBlocks(15, y3, y1, 0, Material.AIR);
		chunk.setBlocks(0, y3, y1, 15, Material.AIR);
		chunk.setBlocks(15, y3, y1, 15, Material.AIR);
		
		// now the top bit
		y1 = y1 - mazeFloorHeight + 1;
...
//		for (int y = 16; y < blockYs.averageHeight + 16; y = y + 16) {
//			generateYSlab(chunk, y + chunkOdds.getRandomInt(3) * 4);
//			if (chunkOdds.flipCoin())
//				generateXSlab(chunk, chunkOdds.getRandomInt(3) * 4, y - 8);
//			if (chunkOdds.flipCoin())
//				generateZSlab(chunk, y - 8, chunkOdds.getRandomInt(3) * 4);
//		}
	}
	
//	private void generateYSlab(RealChunk chunk, int y) {
//		MazeArray maze = new MazeArray(chunkOdds, mazeWidth, mazeWidth);
//		for (int m = 1; m < mazeWidth; m++)
//			for (int n = 1; n < mazeWidth; n++)
//				if (maze.getBit(m, n) == MazeBit.HALL) {
//					int x1 = m * 2 - 1;
//					int z1 = n * 2 - 1;
//					if (chunk.isType(x1, y + 1, z1, wallMaterial))
//						chunk.setBlocks(x1, x1 + 2, y + 1, y + 3, z1, z1 + 2, getRandomOre());
//				}
//	}
//	
//	private void generateXSlab(RealChunk chunk, int x, int y) {
//		MazeArray maze = new MazeArray(chunkOdds, mazeWidth, mazeWidth);
//		for (int m = 1; m < mazeWidth; m++)
//			for (int n = 1; n < mazeWidth; n++)
//				if (maze.getBit(m, n) == MazeBit.HALL) {
//					int y1 = m * 2 - 1 + y;
//					int z1 = n * 2 - 1;
//					if (chunk.isType(x + 1, y1, z1, wallMaterial))
//						chunk.setBlocks(x + 1, x + 3, y1, y1 + 2, z1, z1 + 2, getRandomOre());
//				}
//	}
//	
//	private void generateZSlab(RealChunk chunk, int y, int z) {
//		MazeArray maze = new MazeArray(chunkOdds, mazeWidth, mazeWidth);
//		for (int m = 1; m < mazeWidth; m++)
//			for (int n = 1; n < mazeWidth; n++)
//				if (maze.getBit(m, n) == MazeBit.HALL) {
//					int y1 = m * 2 - 1 + y;
//					int x1 = n * 2 - 1;
//					if (chunk.isType(x1, y1, z + 1, wallMaterial))
//						chunk.setBlocks(x1, x1 + 2, y1, y1 + 2, z + 1, z + 3, getRandomOre());
//				}
//	}
