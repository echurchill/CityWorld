...
public class CityWorldChunkGenerator extends ChunkGenerator {
	...
	private final static double noiseHorizontalScale = 1.0 / 16.0;
	private final static int noiseVerticalScale = 5;
	
	private final static double featureFrequency = 1.50;
	private final static double featureAmplitude = 0.50;
	...
	public byte[][] generateBlockSections(World aWorld, Random random, int chunkX, int chunkZ, BiomeGrid biomes) {
		...
		for (int x = 0; x < byteChunk.width; x++) {
			for (int z = 0; z < byteChunk.width; z++) {
				...
				int noiseY = NoiseGenerator.floor(noise * noiseVerticalScale + feature * featureVerticalScale);
				...
				// calculate the Ys
				int seaY = byteChunk.sealevel + NoiseGenerator.floor(sea * (byteChunk.sealevel - fudgeVerticalScale)) + noiseY;
				int landY = byteChunk.sealevel + NoiseGenerator.floor(land * (byteChunk.height - byteChunk.sealevel - fudgeVerticalScale)) + noiseY;
				...
				if (landY <= byteChunk.sealevel) {
					...
					if (seaY >= byteChunk.sealevel) {
						...
						// if we are too near the sea then we must be a sea level
						if (seaY <= byteChunk.sealevel + 3) {
							y = byteChunk.sealevel;
						}
								
					// if land is higher than the seabed use land to smooth out under water mountain bases
					} else if (landY > seaY) {
	...
	private void generateCrust(ByteChunk byteChunk, int x, int z, byte baseId, int baseY, 
			byte substrateId, int substrateY, byte surfaceId, boolean surfaceCaves) {
	...
	private void generateCrust(ByteChunk byteChunk, int x, int z, byte baseId, int baseY, 
			byte substrateId, int substrateY, byte surfaceId, int coverY, byte coverId, boolean surfaceCaves) {
	...
	private byte getOre(ByteChunk byteChunk, int blockX, int blockY, int blockZ, byte defaultId) {
